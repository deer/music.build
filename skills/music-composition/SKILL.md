...
- **`harmony.comp`** styles: `quarter_stabs` (beats 2&4 — jazz default), `on_beat` (all beats — rock/gospel), `eighth_pump` (8 per bar — funk), `shell_voicings` (root+7th on 2&4 — sparse jazz), `charleston` (dotted-quarter+eighth+quarter — classic jazz). Use octave 3 for piano/Rhodes, octave 4 for guitar.
